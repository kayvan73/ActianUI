...
def infer_image( graph, img ):
    ...
    for i in range( 0, NUM_PREDICTIONS ):
        #the line i commented below is the stock output given by intel
        #print( "%3.1f%%\t" % (100.0 * output[ order[i] ] )
        #       + labels[ order[i] ] )
        guesses = labels[order[i]]
        #print(guesses)
        inferanceArray.append(guesses.split(',')) #split turns comma seperated strings
        #into lists with discrete elements 'a,b,c' -> ['a', 'b', 'c']

    print( "==============================================================" )
    print(inferanceArray[0][0])
    #print(inferanceArray)
    #print(len(inferanceArray))
    return(inferanceArray)

    #========== COMMENTING OUT THE CODE THAT DISPLAYS IMAGE, VERY SLOW ======
...
def main():
    ...
    #img_draw = skimage.io.imread( ARGS.image )  -- i commented out this line to run inferance on my own image
    #it was very easy to do - i just replace the ARGS.image w/ an absolute path to my jpg image
    img_draw = skimage.io.imread( '/home/pi/Desktop/foo2.jpg' )
    img = pre_process_image( img_draw )
    inferanceOutput = infer_image( graph, img )
